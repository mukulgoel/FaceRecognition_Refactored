...
public class FaceBundle implements Serializable, Comparable {
	 ...
	 */
	private static final long serialVersionUID = 7270525396914219100L;
	private static final String NO_IMAGE_SUPPLIED_ERROR = "No image supplied";
	...
	public FaceBundle(double[] avgF, double weightMatrix[][], double[][] eigV,
			String[] files) {

		avgFace = new double[avgF.length];
		this.weightMatrix = new double[weightMatrix.length][weightMatrix[0].length];
		...
		System.arraycopy(weightMatrix, 0, this.weightMatrix, 0,
				weightMatrix.length);
	...
	public double calculateEigenDistance() {
	...
	public String toString() {
		...
		return NO_IMAGE_SUPPLIED_ERROR;
	}
	...
	private void compute() {
		...
		int MAGIC_NR = weightMatrix[0].length;
		int j, pix, image;
		...
		for (image = 0; image < nrfaces; image++) {
			temp = 0.0;
			for (j = 0; j < MAGIC_NR; j++) {
				distance[j] = Math.abs(input_wk[j] - weightMatrix[image][j]);
